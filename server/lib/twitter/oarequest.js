 ...
 */
OARequest = function(oauth, method, path, params) {
...
OARequest.prototype.end = function (callback) {
  this.makeRequest(function(err, raw, response) {
    ...
    try {
      ...
      if (!parsed) {
        var error = new Error('twitter sent empty reply: `'+parsed+'`.')
        error.twitterReply = parsed
        return callback(error, parsed, response)
      }
      return callback(error, parsed, response)
    }
  })
